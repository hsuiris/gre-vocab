...
| 概念清單怎麼定 | 先由 AI 擬一份固定清單，使用者過目後才分字。實際擬出 119 個（原估 60–80 太少，一組會塞到 40 個字） |
...
1. **第一輪 — 擬概念清單。** 8 個 subagent，各拿 ~400 個字的 `word + meaning`，各自回報看到的概念。主流程合併去重、配對相反概念，產出 119 個概念的清單。**使用者過目、刪改後才進第二輪。**
